...
//                 <p>We&apos;re looking for a {job.title.toLowerCase()} to join our team.</p>
//                 <div className="mt-2 flex gap-3 flex-wrap">
//                   <span className="flex items-center gap-1 px-3 py-1 border rounded-full text-sm">
//                     <IoLocationOutline /> 100% Remote
//                   </span>
...
//   const filteredJobs = selectedCategory === "View all" ? jobListings : jobListings.filter(job => job.category === selectedCategory);
  ...
  return (
    <div className="w-full mx-auto p-4">
      ...
      {isPopupOpen && selectedJob && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md relative">
            <button onClick={closePopup} className="absolute top-2 right-2 text-gray-500 hover:text-black">&times;</button>
            {showThankYou ? (
              <h2 className="text-xl font-semibold text-center">Thanks for connecting with us!</h2>
            ) : (
              <>
